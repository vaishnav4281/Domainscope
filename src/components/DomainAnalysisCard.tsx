...
const DomainAnalysisCard = ({ onResults, onMetascraperResults, onVirusTotalResults }: DomainAnalysisCardProps) => {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      // Fallback: use VirusTotal categories for Country if IPQS didn't provide it
      if (locCountry === '-' && attrs.categories) {
        const cats = Object.values(attrs.categories || {}).join(' ').toLowerCase();
        // VT categories sometimes include country hints
      }
